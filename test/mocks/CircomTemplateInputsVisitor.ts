...
export class CircomTemplateInputsVisitor extends CircomVisitor<void> {
  ...
  visitSignalIdentifier = (ctx: SignalIdentifierContext) => {
    ...
    if (
      !ctx.parentCtx &&
      !(ctx.parentCtx! instanceof SignalIdentifierListContext)
    ) {
      throw new Error(
        "INTERNAL ERROR: SignalIdentifier should have a SignalIdentifierListContext as a parent",
      );
    }

    if (
      !ctx.parentCtx!.parentCtx &&
      !(ctx.parentCtx!.parentCtx! instanceof SignalDeclarationContext)
    ) {
      throw new Error(
    ...
    if (signalDeclarationContext.signalHeader().SIGNAL_TYPE()) {
      signalType = signalDeclarationContext
        .signalHeader()
        .SIGNAL_TYPE()
        .getText();
    }
  ...
  visitVarIdentifier = (ctx: VarIdentifierContext) => {
    const identifierObjects = this._resolveIdentifier(
      ctx.identifier(),
      this._vars,
    );
